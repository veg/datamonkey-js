...
exports.invoke = function(req, res) {
  ...
  Msa.parseFile(fn, datatype, gencodeid, function(err, msa) {
    ...
    if(msa.sites > fubar.max_sites) {
      var error = 'Site limit exceeded! Sites must be less than ' + fubar.max_sites;
      logger.error(error);
    ...
    if(msa.sequences > fubar.max_sequences) {
      var error = 'Sequence limit exceeded! Sequences must be less than ' + fubar.max_sequences;
      logger.error(error);
      res.json(500, {'error' : error});
      return;
    }
...
exports.getInfo = function(req, res) {
  ...
  //Return all results
  FUBAR.findOne({_id : id}, {creation_time: 1, start_time: 1, status: 1}, function(err, fubar_info) {
    if (err || !fubar_info) {
      logger.error(err);
