...
exports.invoke = function(req, res) {
  ...
  Msa.parseFile(fn, datatype, gencodeid, function(err, msa) {
    ...
    if(msa.sites > slac.max_sites) {
      var error = 'Site limit exceeded! Sites must be less than ' + slac.max_sites;
      logger.error(error);
    ...
    if(msa.sequences > slac.max_sequences) {
      var error = 'Sequence limit exceeded! Sequences must be less than ' + slac.max_sequences;
      logger.error(error);
      res.json(500, {'error' : error});
      return;
    }
...
exports.getInfo = function(req, res) {
  ...
  //Return all results
  SLAC.findOne({_id : id}, {creation_time: 1, start_time: 1, status: 1}, function(err, slac_info) {
    if (err || !slac_info) {
      logger.error(err);
