...
function perform_the_mapping (headers, publisher_push) {
    ...
    function isCountry(supposed_country) {
        return supposed_country in attributes.country.value ? [attributes.types.COUNTRY, supposed_country] : null;
    }
    ...
    function isSubtype(supposed_subtype) {
        return supposed_subtype in subtype_dictionary ? [attributes.types.SUBTYPE, supposed_subtype] : null;
    }
    ...
    function isDate(supposed_date) {
        ...
        return [attributes.types.DATE, "" + parsed_date.getFullYear() + "" + (1 + parsed_date.getMonth()) + "" + parsed_date.getDate()];
    ...
    var binned_by_delimiter = attributes.delimiters.map(function(delimiter, index) {
        var fields = [];
    ...
    binned_by_delimiter.forEach(function(delimiter_record, index) {
        ...
        delimiter_record.push(_.reduce(delimiter_record[1].map(function(mapping) {
