...
/////ADD MY FUNC HERE
//get url from my function
//Added url as parameter in self.initializeServer
  loadBalance(cluster_ip_urls_array, (err, return_url) => {
    var url = return_url;
    console.log(url);
    self.initializeServer(url, function() {
      if(action == 'spawn') {
        self.spawn();
...
//This should go into my function call because URL is an argument that
//comes from my function.
  }
...
HPCSocket.prototype.handleError = function (data) {
  ...
  self.job.analysis.save( function (err, result) {
    if (err) throw err;
    publisher.publish(self.channel,
...
HPCSocket.prototype.connectionError = function (err) {
  ...
  err = "Problem connecting to " + setup.cluster_ip + ". " + err;
  logger.error(err);
...
HPCSocket.prototype.initializeServer = function (url,callback) {

  var self = this;

  self.server_socket = ioclient.connect(url, {'connect timeout': 1000,
                                                          'force new connection': true,
                                                          'reconnection': false});

  self.server_socket.on('connect_error', function() { self.connectionError() });
  self.server_socket.on('connect_timeout', function() { self.connectionError() });
  self.server_socket.on('error', function() { self.connectionError() });
  ...
  self.server_socket.on('status update', function (data) {
    ...
    self.job.analysis.creation_time = data.ctime;
    self.job.analysis.start_time = data.stime;
  ...
  self.server_socket.on('aligned fasta', function (data) {
    ...
    fs.writeFile(self.job.analysis.aligned_fasta_fn, data.buffer, function (err) {
      if (err) throw err;
...
HPCSocket.prototype.spawn = function (callback) {
  ...
  self.server_socket.on('connected', function (data) {
    ...
    job_to_send = { 'filepath'    : self.job.filepath,
                    'msa'         : self.job.msa,
    ...
    ss(self.server_socket).emit(self.job.action, stream, { job : job_to_send,
                                              status_stack : self.job.status_stack });
...
ClusterStatus.prototype.initializeServer = function (callback) {
