...
HPCSocket.prototype.connectionError = function (err) {
  ...
  err = "Problem connecting to " + default_setup_url + ". " + err;
  logger.error(err);
...
HPCSocket.prototype.initializeServer = function (url_input, callback) {
  ...
  self.server_socket = ioclient.connect(url_input, {'connect timeout': 1000,
...
ClusterStatus.prototype.initializeServer = function (url, allback) {

  var self = this;

  var server_socket = ioclient.connect(self.url, {'connect timeout': 1000,
                                                          'force new connection': true,
                                                          'reconnection': false});
...
HPCSocketRecheck.prototype.initializeServer = function (job, callback) {
  ...
  var self = this;
  var server_socket = ioclient.connect(default_setup_url, {'connect timeout': 1000,
                                                          'force new connection': true,
                                                          'reconnection': false});
