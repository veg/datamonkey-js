...
HPCSocket.prototype.initializeServer = function (url_input, callback) {
  ...
  // tn93 file
  self.server_socket.on('tn93', function (data) {
    // write buffer out to file
    fs.writeFile(self.job.analysis.tn93_fn, data.buffer, function (err) {
      if (err) throw err;
      winston.info('saved tn93 file');
     });
  });
...
ClusterStatus.prototype.initializeServer = function (url, callback) {
  ...
  var server_socket = ioclient.connect(url, {'connect timeout': 1000,
                                                          'force new connection': true,
                                                          'reconnection': false});
