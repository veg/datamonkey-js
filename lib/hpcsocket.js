const path = require("path"),
  ...
  difFubarMarkdown = require(path.join(__dirname, "difFubarMarkdownWrapper.js")),
...
const loadBalance = require("./../lib/loadBal.js").loadBalance;
const default_setup_url = setup.default_url;
...
function HPCSocket(job, action, callback) {
  ...
  loadBalance(cluster_ip_urls_array, (err, return_url) => {
    ...
    if (self.job.analysis.analysistype == "flea") {
      ...
      winston.info("Using FLEA backend: " + self.url);
    }

    if (self.job.analysis.analysistype == "difFubar") {
      self.url = setup.difFubar_ip_address;
      winston.info("Using difFUBAR backend: " + self.url);
    }

    winston.info("Connecting to backend URL: " + self.url + " for " + self.job.analysis.analysistype);
...
HPCSocket.prototype.handleError = function (data) {
  ...
  if (self.job.analysis.mail && !self.job.analysis.notified) {
    mailer.sendJobError(self.job.analysis);
    self.job.analysis.notified = true;
  }

  self.job.analysis.stdout = data.stdout ? data.stdout : "";
  self.job.analysis.stderr = data.stderr ? data.stderr : "";
  // Apply markdown wrapper for error messages in difFUBAR analyses
  let errorMsg = data.progress ? data.progress : "";
  if (self.job.analysis.analysistype === "difFubar" && errorMsg) {
    errorMsg = difFubarMarkdown.wrapDifFubarStatus(errorMsg);
  }
  self.job.analysis.last_status_msg = errorMsg;
...
HPCSocket.prototype.initializeServer = function (url_input, callback) {
  ...
  self.server_socket.on("status update", function (data) {
      ...
      // Apply markdown wrapper for difFUBAR analyses
      let statusMsg = data.msg;
      if (self.job.analysis.analysistype === "difFubar") {
        statusMsg = difFubarMarkdown.wrapDifFubarStatus(data.msg);
      }
      self.job.analysis.last_status_msg = statusMsg;
  ...
  self.server_socket.on("aligned fasta", function (data) {
    ...
    fs.writeFile(
      self.job.analysis.aligned_fasta_fn,
      data.buffer,
      function (err) {
        if (err) throw err;
  ...
  self.server_socket.on("gard nexus file", function (data) {
    // write buffer out to file
    fs.writeFile(
      self.job.analysis.result_nexus_fn,
      data.buffer,
      function (err) {
        if (err) {
          winston.info(err);
  ...
  self.server_socket.on("flea session json file", function (data) {
    // write buffer out to file
    fs.writeFile(
      self.job.analysis.session_json_fn,
      data.buffer,
      function (err) {
        if (err) {
          winston.info(err);
  ...
  // difFUBAR overview png
  self.server_socket.on('difFubar overview png', function(data) {
    const imageBuffer = data.buffer;
    if (self.job.analysis.overview_plot_png_fn) {
      fs.writeFile(self.job.analysis.overview_plot_png_fn, imageBuffer, function(err) {
        if (err) {
          winston.info(err);
        } else {
          winston.info("saved difFUBAR overview plot PNG file");
        }
      });
    }
  });

  // difFUBAR overview svg
  self.server_socket.on('difFubar overview svg', function(data) {
    const svgBuffer = data.buffer;
    if (self.job.analysis.overview_plot_svg_fn) {
      fs.writeFile(self.job.analysis.overview_plot_svg_fn, svgBuffer, function(err) {
        if (err) {
          winston.info(err);
        } else {
          winston.info("saved difFUBAR overview plot SVG file");
        }
      });
    }
  });

  // difFUBAR posteriors png
  self.server_socket.on('difFubar posteriors png', function(data) {
    const imageBuffer = data.buffer;
    if (self.job.analysis.posterior_plots_png_fn) {
      fs.writeFile(self.job.analysis.posterior_plots_png_fn, imageBuffer, function(err) {
        if (err) {
          winston.info(err);
        } else {
          winston.info("saved difFUBAR posterior plots PNG file");
        }
      });
    }
  });

  // difFUBAR posteriors svg
  self.server_socket.on('difFubar posteriors svg', function(data) {
    const svgBuffer = data.buffer;
    if (self.job.analysis.posterior_plots_svg_fn) {
      fs.writeFile(self.job.analysis.posterior_plots_svg_fn, svgBuffer, function(err) {
        if (err) {
          winston.info(err);
        } else {
          winston.info("saved difFUBAR posterior plots SVG file");
        }
      });
    }
  });

  // difFUBAR detections png
  self.server_socket.on('difFubar detections png', function(data) {
    const imageBuffer = data.buffer;
    if (self.job.analysis.detection_plots_png_fn) {
      fs.writeFile(self.job.analysis.detection_plots_png_fn, imageBuffer, function(err) {
        if (err) {
          winston.info(err);
        } else {
          winston.info("saved difFUBAR detection plots PNG file");
        }
      });
    }
  });

  // difFUBAR detections svg
  self.server_socket.on('difFubar detections svg', function(data) {
    const svgBuffer = data.buffer;
    if (self.job.analysis.detection_plots_svg_fn) {
      fs.writeFile(self.job.analysis.detection_plots_svg_fn, svgBuffer, function(err) {
        if (err) {
          winston.info(err);
        } else {
          winston.info("saved difFUBAR detection plots SVG file");
        }
      });
    }
  });

  // difFUBAR results file
  self.server_socket.on('difFubar results file', function(data) {
    const resultsBuffer = data.buffer;
    if (self.job.analysis.results_path) {
      fs.writeFile(self.job.analysis.results_path, resultsBuffer, function(err) {
        if (err) {
          winston.info(err);
        } else {
          winston.info("saved difFUBAR results JSON file");
        }
      });
    }
  });
  ...
  // Progress file
  self.server_socket.on("progress file", function (params, stream) {
    var data = "";
  ...
  self.server_socket.on("completed", function (data) {
    ...
    if (self.job.analysis.onComplete) {
      ...
      if (data) {
        fs.writeFile(
          self.job.analysis.results_path,
          data.results,
          function (err) {
            if (err) winston.warn(err);
...
HPCSocket.prototype.spawn = function (callback) {
  ...
  self.server_socket.on("connected", function (data) {
    ...
    //Send file
    fs.readFile(self.job.filepath, (err, data) => {
      self.server_socket.emit(self.job.action, data.toString(), {
        job: job_to_send,
        status_stack: self.job.status_stack,
      });
    });
...
HPCSocket.prototype.check = function () {
  var self = this;
  ...
  job_to_send = {
    ...
    status_stack: self.job.status_stack,
    type: self.job.type,
    torque_id: self.job.torque_id,
  };

  self.job.action = self.job.type + ":check";

  // Once connected to the server, prepare the request and spawn the item.
  self.server_socket.on("connected", function (data) {
    self.server_socket.emit(self.job.action, { job: job_to_send });
  });
};
