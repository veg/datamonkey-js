...
function queueSet() {
  ...
  new hpcsocket.JobQueue((err, job_queue) => {
    if (err) {
      winston.warn("hpcsocket had an error getting the jobQueue: ");
      winston.warn(err);
      return;
    }
    client.set(jobQueueRedisKey, JSON.stringify(job_queue), (err, reply) => {
      if (err) {
        winston.warn("redis client had an error setting the jobQueue: ");
        winston.warn(err);
        return;
      }
      return;
...
function queueGet(callback) {
  ...
  client.get(jobQueueRedisKey, (err, reply) => {
    if (err) {
      winston.warn("redis client had an error getting the jobQueue: ");
      winston.warn(err);
      callback(err, null);
      return;
    }
