$( document ).ready( function () {
  ...
  if(user_tree_nwk) {
    fade_create_neighbor_tree(user_tree_nwk);
  } else {
    fade_create_neighbor_tree(nj_nwk);
  }
...
function fade_create_neighbor_tree(nwk) {
...
$("form").submit(function(e) {
  ...
  validate_selection(tree, function(err) {
    if(err) {
      ...
      var fg_branches = tree.get_selection().map(function(branch) { return branch.name}).join(';');
      formData.append('nwk_tree', nwk_tree);
      formData.append('fg_branches', fg_branches);
      ...
      xhr.onload = function(res) {
        ...
        if('_id' in result.fade) {
          window.location.href =  '/fade/' + result.fade._id;
