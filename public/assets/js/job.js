...
function jobRuntime(start_time) {
  ...
  d3.select("#job-run-time").text(
    zero(hours) + ":" + zero(mins) + ":" + zero(secs)
  );
}
...
function setupJob() {
  ...
  d3.json(jobid + "/info", function (data) {
    if (data.status == "aborted") {
      ...
      var job_start_time = moment(data.start_time);
      var job_creation_time = moment(data.creation_time);

      if (!_.isEmpty(data.start_time)) {
