$(document).ready(function() {
  ...
  if ("#tree-select-btn-group") {
    $("#tree-select-btn-group").find(".btn").each(function(i, obj) {
...
function relax_create_neighbor_tree(nwk) {
  ...
  $("#relax-highlighter-btn-group").find(".btn").each(function(i, obj) {
    $(obj).on("click", function(d) {
      current_selection_name = $(this).data("name");
      tree.selection_label(current_selection_name);
    });
...
$("form").submit(function(e) {
  ...
  var validate_selection = function(tree, callback) {
    ...
    var at_least_one_test = tree.nodes.get_nodes().some(function(d) {
      return d.test;
    });
    var at_least_one_reference = tree.nodes.get_nodes().some(function(d) {
      return d.reference;
    });

    if (!tree.nodes.get_selection().length) {
      callback({
        msg: "No branch selections were made, please select one. Alternatively, you can choose to select all via the menu."
      });
    } else if (!at_least_one_test) {
      callback({
        msg: "No test branch selections were made, please select one."
      });
