before_48_hours = new Date(new Date().getTime()-60*60*48*1000).toISOString()

pending_absrels = db.absrels.find({status:"running", created: {$lte : before_48_hours}}, {created: 1, torque_id : 1, status: 1}).toArray();
pending_busteds = db.busteds.find({status:"running", created: {$lte : before_48_hours}}, {created: 1, torque_id : 1, status: 1}).toArray();
pending_relaxes = db.relaxes.find({status:"running", created: {$lte : before_48_hours}}, {created: 1, torque_id : 1, status: 1}).toArray();
pending_fels = db.fels.find({status:"running", created: {$lte : before_48_hours}}, {created: 1, torque_id : 1, status: 1}).toArray();
pending_fubars = db.fubars.find({status:"running", created: {$lte : before_48_hours}}, {created: 1, torque_id : 1, status: 1}).toArray();
pending_gards = db.gards.find({status:"running", created: {$lte : before_48_hours}}, {created: 1, torque_id : 1, status: 1}).toArray();
pending_memes = db.memes.find({status:"running", created: {$lte : before_48_hours}}, {created: 1, torque_id : 1, status: 1}).toArray();
pending_slacs = db.slacs.find({status:"running", created: {$lte : before_48_hours}}, {created: 1, torque_id : 1, status: 1}).toArray();
